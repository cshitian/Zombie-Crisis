...
### 火箭筒武器爆炸效果增强 (Rocket Launcher Explosion Enhancement)

- **爆炸物理效果增强 (Enhanced Explosion Physics)**:
  - **多重弹坑系统**: 现在火箭筒在大规模爆炸后会在落点周围产生 3-5 个小型随机弹坑，模拟二次爆炸与地表破坏，参考空袭任务效果实现。
- **建筑损害与动态可视化 (Building Damage & Visuals)**:
  - **火灾与坍塌判定**: 火箭筒打击范围内（扩展至 2 倍溅射半径）的建筑现在有几率触发火灾 (40%) 或坍塌 (20%)。
  - **动态动画图标**:
    - **火灾动画**: 为起火建筑新增了动态火焰图标 (🔥)，采用心跳式缩放动画。
    - **坍塌状态**: 为受损建筑新增了永久性的坍塌图标 (🏚️)。
- **自动熄灭机制**: 实现了火焰图标的动态缩小逻辑，火苗会随时间逐渐减弱并在 30 秒后完全熄灭消失，而坍塌标记将永久保留在战场。
